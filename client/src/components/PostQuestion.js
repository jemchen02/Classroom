...
export default class PostQuestion extends React.Component{
	...
	async handlePostCreate() {
		...
		try {
			if(content.trim() == "") throw "You must write a question.-"
		}
		catch(err) {
		...
		try {
			if(answer.trim() == "") throw "You must write an answer to the question.-"
		}
		catch(err) {
			this.setState(prevState => ({message: prevState.message + err}));
			post = false;
		...
		if(post) {
			var newPost;
			const mongoPost = {postedBy: this.props.user.name, content:content, answer:answer, className: this.props.Class.name}
	...
	render() {
		...
		return(
			<div>
				{messageLog}
				<br></br>
				<p class = "formH">New Question</p>
				<p class = "form">Question text</p>
				<input id = "postContent" type = "text"></input>
				<br></br>
				<p class = "form">Answer</p>
